...
ddd = enchant.Dict("en-US") if enchant else _DummyDictionary()


class SignLanguageEngine:
    """Stateful sign-language recognizer extracted from the original Tkinter app."""

    def __init__(self, model_path="cnn8grps_rad1_model.h5", offset=29):
    ...
    def _state_dict(self, has_hand, skeleton_image=None):
        return {
            "has_hand": has_hand,
            ...
            "group_top3": self.group_top3,
            "skeleton_image": skeleton_image,
        }

    def _update_suggestions(self):
        """Recompute word suggestions based on the current sentence."""
    ...
    def process_frame(self, frame_bgr):
        with self.lock:
            ...
            ok, buffer = cv2.imencode(".jpg", skeleton)
            skeleton_b64 = base64.b64encode(buffer).decode("utf-8") if ok else None

            return self._state_dict(True, skeleton_b64)
    ...
    def apply_suggestion(self, idx):
        with self.lock:
            options = [self.word1, self.word2, self.word3, self.word4]
            if idx < 0 or idx > 3:
                return self._state_dict(False)

            selected = options[idx].strip()
            if not selected:
                return self._state_dict(False)
    ...
    def predict(self, test_image):
        ...
        prob[ch3] = 0
        self.group_top3 = [int(ch1), int(ch2), int(ch3)]
